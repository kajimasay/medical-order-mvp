// Files API with Blob storage + fallback
import { getFiles } from '../lib/kv-database.js';
...
// Initialize files data with persistent demo data
function initializeFiles() {
  // Always ensure we have baseline demo files, even after redeployment
  const demoFiles = [
    {
  ...
  if (global.globalFilesStorage === null || global.globalFilesStorage === undefined || !Array.isArray(global.globalFilesStorage)) {
    console.log('Initializing global file storage with demo files...');
    global.globalFilesStorage = [...demoFiles];
  } else {
    // Ensure demo files always exist (merge with existing files)
    console.log('Merging with existing global file storage...');
    demoFiles.forEach(demoFile => {
      const existingFile = global.globalFilesStorage.find(f => f.id === demoFile.id);
      if (!existingFile) {
        global.globalFilesStorage.push(demoFile);
        console.log('Added missing demo file:', demoFile.id);
      }
    });
  }
    
  // Add demo content to files that don't have content
...
function getFiles() {
  try {
    // Force sync with current global storage state
    const files = syncWithUploadedFiles();
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (useBlobStorage) {
        try {
          console.log('Attempting to get files from Blob...');
          const allFiles = await getFiles();
          files = [...allFiles];
          console.log('All files from Blob:', files.map(f => ({ id: f.id, orderId: f.orderId, name: f.originalName })));
        } catch (blobError) {
          console.error('Blob files error, falling back to memory:', blobError);
          files = [...fallbackFiles];
          useBlobStorage = false;
        }
      } else {
        files = [...fallbackFiles];
        console.log('All files from fallback memory:', files.map(f => ({ id: f.id, orderId: f.orderId, name: f.originalName })));
      }
